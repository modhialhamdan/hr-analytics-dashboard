...
with col3:  # left column
    st.markdown("**3) Attrition Distribution (Filtered)**")  # chart title
    if "Attrition" in df_view.columns:  # check column
...
with col4:  # right column
    st.markdown("**4) Income Distribution by Department (Filtered)**")  # chart title
...
# Data Table (Filtered view)

st.subheader("Employee Table (Filtered)")  # section header
...
action_col1, action_col2 = st.columns(2)  # split into two action panels


# Action 1: Add new employee 
with action_col1:  # left action panel
...
with action_col2:  # right action panel
